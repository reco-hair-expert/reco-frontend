...
describe("AddressForm Component", () => {
  ...
  it("renders payment icons", () => {
    render(<AddressForm {...defaultProps} />);
    const paymentIcon = screen.getByAltText(
      "Visa and Mastercard payment options"
    );
    expect(paymentIcon).toBeInTheDocument();
    expect(paymentIcon).toHaveAttribute(
      "src",
      "/images/sections/footer/visa-mastercard.svg"
    );
  });
  ...
  it("applies custom className when provided", () => {
    ...
    const { container } = render(
      <AddressForm {...defaultProps} className={customClass} />
    );
