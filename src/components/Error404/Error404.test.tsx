...
jest.mock('../BackgroundCircles/BackgroundCircles', () => {
  return ({ className, 'aria-hidden': ariaHidden }: any) => {
    return <div data-testid="background-circles" className={className} aria-hidden={ariaHidden} />;
  };
});
...
describe('Error404 Component', () => {
  it('renders the 404 error page with all elements', () => {
    ...
    expect(image).toHaveAttribute('src', '/images/sections/error/reco-every_day-1x.png');
    
    // Check the error message
    const errorText = screen.getByText('Ой! Ця сторінка явно переживає день неслухняного волосся!');
