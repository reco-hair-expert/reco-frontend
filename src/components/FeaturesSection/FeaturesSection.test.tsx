...
describe("FeaturesSection Component", () => {
  it("renders all features sections with correct content", () => {
    ...
    const section = screen
      .getByRole("heading", { name: "Особливості продукту" })
      .closest("section");
    ...
    const leftText = screen.getByText(
      /навіть для волосся з 5 ступенем пошкодження/i
    );
    ...
    expect(productImage).toHaveAttribute(
      "src",
      "/images/sections/features/recoil.png"
    );

    // Check right side features
    const rightTopHeading = screen.getByRole("heading", {
      name: "Простота у використанні"
    });
    ...
    const rightBottomHeading = screen.getByRole("heading", {
      name: "Комплексний підхід"
    });
    expect(rightBottomHeading).toBeInTheDocument();

    const rightBottomText = screen.getByText(
      /не лише відновлює, але й забезпечує/i
    );
