...
jest.mock('../Button/Button', () => {
  return ({ children, variant, size }: { children: React.ReactNode; variant: string; size: string }) => (
...
describe('NavButtons Component', () => {
  ...
  it('applies custom className when provided', () => {
    ...
    const { container } = render(
      <NavButtons buttons={mockButtons} labels={mockLabels} className={customClass} />
    );
  ...
  it('renders with default className when no custom className is provided', () => {
    const { container } = render(<NavButtons buttons={mockButtons} labels={mockLabels} />);
    const navButtonsContainer = container.firstChild;
